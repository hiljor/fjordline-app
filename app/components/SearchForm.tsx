...
/**
 * A versatile search form for getting information about a booking request.
 * @param param0
 * @returns
 */
  ...
  return (
    <div className="relative w-full z-[100]">
      ...
      {collapsible && (
        <button
          onClick={() => {
            // If we are currently editing and about to close, reset the state to match the URL
            if (isEditing) {
              setIsRoundTrip(!!urlReturnDate);
            }
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="flex-shrink-0 px-4 flex flex-col items-center gap-1">
              {!!urlReturnDate ? (
                <ArrowRightLeft
                  className="text-slate-200 group-hover:text-brand transition-colors"
                  size={34}
                />
              ) : (
                <ArrowRight
              ...
              <span className="text-[8px] font-black text-slate-300 uppercase tracking-widest group-hover:text-brand/50">
                {!!urlReturnDate ? "Tur/Retur" : "En vei"}
              </span>
            </div>
        ...
        {isEditing && (
          ...
          >
            <div className={`${collapsible ? "mt-6 mb-8" : ""} p-1`}>
              <div
                className={`w-full bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 ${!collapsible && "max-w-4xl mx-auto shadow-xl"}`}
              >
                {/* Tabs */}
                ...
                >
                  ...
                  {[
                    {
                      ...
                      name: "from",
                      val: urlFrom,
                      options: ["Bergen", "Stavanger", "Hirtshals"],
                    },
                    {
                      label: "Til",
                      name: "to",
                      val: urlTo,
                      options: ["Stavanger", "Hirtshals", "Bergen"],
                    },
                  ].map((field) => (
                  ...
                  <div
                    className={`flex flex-col gap-2 transition-opacity ${!isRoundTrip ? "opacity-30 pointer-events-none" : "opacity-100"}`}
                  >
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
                      Retur
                    </label>
